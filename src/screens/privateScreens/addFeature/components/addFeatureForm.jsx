    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-sky-50 to-teal-50 p-6">
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <InputField label="Feature Name" name="title" value={formData.title} onChange={handleChange} placeholder="Enter feature name" maxLength={255} />

          <TextareaField label="Feature Description" name="description" value={formData.description} onChange={handleChange} placeholder="Enter feature description" maxLength={512} />

          <InputField label="Due Date" type="date" name="duedate" value={formData.duedate} onChange={handleChange} min={new Date().toISOString().split("T")[0]} />
...
const InputField = ({ label, type = "text", maxLength, ...props }) => {
...
const TextareaField = ({ label, maxLength, ...props }) => (
  <div>
    <label className="block text-sm font-medium text-slate-700 mb-1">{label}</label>
    <textarea
